  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      try {
        ...
        if (user) {
          // Pokušavamo da nađemo full_name iz metapodataka, ili uzimamo početno slovo email-a
    ...
    <header className="sticky top-0 z-50 bg-[var(--bg-soft)] border-b border-[var(--accent-soft)]">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between relative z-20">
        ...
        <div className="flex-shrink-0">
          <Link href="/" onClick={() => setIsMenuOpen(false)} className="text-xl md:text-2xl font-extrabold tracking-tight text-[var(--text-main)]">
            Emocionalna <span className="text-[var(--primary)]">Pismenost.</span>
          </Link>
        </div>

        <nav className="hidden md:flex items-center gap-10 text-sm font-bold uppercase tracking-wider text-[var(--text-main)]">
